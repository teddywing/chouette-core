...
CREATE OR REPLACE FUNCTION objectid_short_id_as_int(text)
...
CREATE OR REPLACE FUNCTION objectid_short_id_as_text(text)
RETURNS text LANGUAGE sql AS $$
  SELECT (string_to_array(shared_extensions.objectid_short_id_before_last_fragment($1), '-'))[array_upper(string_to_array(shared_extensions.objectid_short_id_before_last_fragment($1), '-'), 1)]::text;
$$;
