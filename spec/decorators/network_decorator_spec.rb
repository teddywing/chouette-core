...
describe NetworkDecorator do
  let(:object) { build_stubbed :network }
  let(:user) { build_stubbed :user }

  describe 'action links' do
    before do
      allow(helpers).to receive(:edit_line_referential_network_path).and_return('/edit_path')
      allow(helpers).to receive(:new_line_referential_network_path).and_return('/new_path')
      allow(helpers).to receive(:line_referential_network_path).and_return('/show_path')
