...
describe ReferentialNetworkDecorator do
  let(:object) { build_stubbed :network }
  let(:user) { build_stubbed :user }

  describe 'action links' do
    subject { ReferentialNetworkDecorator.decorate(object) }

    before do
      allow(helpers).to receive(:edit_referential_network_path).and_return('/edit_path')
      allow(helpers).to receive(:new_referential_network_path).and_return('/new_path')
      allow(helpers).to receive(:referential_network_path).and_return('/show_path')
    ...
    context 'for authorized user' do
      ...
      it 'should render create link' do
        expect(subject.action_links.map(&:content)).to include(I18n.t('networks.actions.new'))
        expect_action_link_hrefs.to include('/new_path')
      end

      it 'should render edit link' do
        expect(subject.action_links.map(&:content)).to include(I18n.t('networks.actions.edit'))
    ...
    context 'for unauthorized user' do
      ...
      it 'should not render create link' do
        expect(subject.action_links.map(&:content)).to_not include(I18n.t('networks.actions.new'))
        expect_action_link_hrefs.to_not include('/new_path')
      end

      it 'should not render edit link' do
        expect(subject.action_links.map(&:content)).to_not include(I18n.t('networks.actions.edit'))
